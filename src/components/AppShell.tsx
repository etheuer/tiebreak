...
export async function AppShell({
  ...
  const nav = [
    ...
    { label: "Comparisons", href: hubHref(market) },
  ];
    ...
    "@graph": [
      {
        ...
        name: SITE_NAME,
        url: SITE_URL,
        logo: `${SITE_URL}/brand/clinchmark-logo.png`,
      },
      {
        "@type": "WebSite",
        "@id": `${SITE_URL}/#website`,
    ...
    <>
      ...
      {publishUk ? (
        <MarketBanner
          ...
          ukCompareSlugs={ukComparisons.map(
            (comparison) => `${comparison.productA}-vs-${comparison.productB}`,
          )}
      ...
      {market === "uk" ? (
        <p className="shell pb-2 text-meta leading-relaxed text-ink-3">
          UK buyers also have rights under the Consumer Rights Act 2015; this
          sheet is manufacturer warranty, not legal advice.
        </p>
      ) : null}
      <SiteFooter
