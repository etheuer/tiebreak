...
export function Monogram({
  ...
  if (size === "hero" || size === "lg")
    return (
      <span
        aria-hidden
        className={`p-illustration ${className}`}
        style={{
          width: spec.box,
          height: spec.box,
          color: tone === "b" ? "var(--rival)" : "var(--accent)",
        }}
      >
        <Glyph subcategory={visual.subcategory} size={spec.glyph} />
      </span>
    );
  const hue = brandHue(visual.brand);
  const initials = brandInitials(visual.brand);
  const showGlyph = size === "md";
  return (
    <span
      aria-hidden
    ...
    >
      <span
        ...
        style={{ fontSize: Math.round(spec.box * 0.38) }}
      >
        {initials}
      </span>
      {showGlyph && (
        <span className="p-glyph">
          <Glyph
            ...
            size={Math.round(spec.glyph * 0.62)}
          />
        </span>
      )}
    </span>
