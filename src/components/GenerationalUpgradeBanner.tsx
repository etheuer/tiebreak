...
export function GenerationalUpgradeBanner({
  ...
  const older = analysis.olderProduct ? shortName(analysis.olderProduct) : productA.name
  const newer = analysis.newerProduct ? shortName(analysis.newerProduct) : productB.name
            ...
            <div>
              ...
              <ul className="mt-2 grid gap-1.5">
                {analysis.whatChanged.map((item) => (
                  <li key={item} className="flex items-start gap-2 text-meta leading-snug text-ink-2">
                    <span className="shrink-0 text-accent font-bold" aria-hidden>
                      +
                    </span>
                    <span>{item}</span>
                  </li>
            ...
            <div>
              ...
              <ul className="mt-2 grid gap-1.5">
                {analysis.whatStayedSame.map((item) => (
                  <li key={item} className="flex items-start gap-2 text-meta leading-snug text-ink-2">
                    <span className="shrink-0 text-ink-3 font-semibold" aria-hidden>
                      =
                    </span>
                    <span>{item}</span>
                  </li>
